...
export default function AIAssistant({ onClose }: AIAssistantProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First try to use the Netlify function for the OpenAI API
      try {
        // Create an AbortController for timeout
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 10000); // Increased timeout for serverless cold starts
      ...
      const responseContent = isStaticDeployment 
        ? `[Using pre-defined responses until API connection is established]\n\n${mockResponse}`
        : mockResponse;
...
1. Your OpenAI API key is correctly set in Netlify environment variables
2. The Netlify function is properly deployed
